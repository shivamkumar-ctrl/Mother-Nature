...
import { ArrowLeft, Package, Truck, MapPin, XCircle } from "lucide-react";
...
export default function CustomerOrderDetail() {
  ...
  const canCancel =
    ...
    differenceInMinutes(new Date(), new Date(order.createdAt)) < 120;

  const minutesLeft = order
    ? Math.max(0, 120 - differenceInMinutes(new Date(), new Date(order.createdAt)))
    : 0;
    ...
    return (
      <Layout>
        <div className="container mx-auto px-4 py-24 text-center">
          ...
          <Link href="/orders" className="text-secondary hover:underline">
            Return to orders
          </Link>
        </div>
      </Layout>
  ...
  return (
    <Layout>
      ...
      <div className="container mx-auto px-4 py-8 max-w-4xl space-y-8">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div>
            <h1 className="text-3xl font-serif text-primary flex items-center gap-3">
              ...
              <Badge
                variant="secondary"
                className={`${getStatusColor(order.status)} border-0 text-sm capitalize`}
              >
                {order.status}
              </Badge>
            </h1>
        ...
        {canCancel && (
          <div className="bg-amber-50 border border-amber-200 rounded-xl px-5 py-3 text-sm text-amber-800 dark:bg-amber-900/20 dark:border-amber-800 dark:text-amber-200">
            You have <strong>{minutesLeft} minute{minutesLeft !== 1 ? "s" : ""}</strong> left to cancel this order.
          </div>
        )}
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="bg-card border rounded-xl p-5 shadow-sm space-y-3">
            ...
            <p className="text-xs text-muted-foreground italic">
              You will be notified when your status changes.
            </p>
          </div>
